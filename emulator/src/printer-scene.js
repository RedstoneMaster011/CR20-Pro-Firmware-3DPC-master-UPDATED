...
const MM = 0.01;
const MAX_FILAMENT_SEGMENTS = 120000;
const BED_CENTER_Z = -1.48;
...
export class PrinterScene {
  constructor(container) {
    ...
    this.camera.position.set(9.2, 6.6, 12.5);
    ...
    this.lastFilamentPoint = null;
    this.filamentSegmentCount = 0;
    this.buildPrinter();
    this.addLighting();
  ...
  buildPrinter() {
    ...
    const bedTop = new THREE.MeshStandardMaterial({ color: 0x353b3d, roughness: 0.86, metalness: 0.1 });
    ...
    this.filamentPositions = new Float32Array(MAX_FILAMENT_SEGMENTS * 6);
    ...
    this.filamentTrace = new THREE.LineSegments(
      this.filamentGeometry,
      new THREE.LineBasicMaterial({ color: 0xf48142 }),
    );
    this.filamentTrace.frustumCulled = false;
    this.bedGroup.add(this.filamentTrace);
    ...
    const probeBody = box(0.13, 0.43, 0.13, black);
    probeBody.position.set(-0.28, -0.64, 0.04);
    this.carriageGroup.add(probeBody);
    this.probePin = cylinder(0.025, 0.42, steel, 12);
    this.probePin.position.set(-0.28, -0.88, 0.04);
    this.carriageGroup.add(this.probePin);

    const fanHousing = cylinder(0.22, 0.055, rubber, 24);
    fanHousing.rotation.x = Math.PI / 2;
    fanHousing.position.set(0.14, -0.24, 0.53);
    this.carriageGroup.add(fanHousing);

    const fanRotorMount = new THREE.Group();
    fanRotorMount.rotation.x = Math.PI / 2;
    fanRotorMount.position.set(0.14, -0.24, 0.565);
    this.fan = new THREE.Group();
    for (let index = 0; index < 5; index += 1) {
      const blade = box(0.055, 0.018, 0.17, edge);
      blade.position.z = 0.08;
      blade.rotation.y = index * Math.PI * 0.4;
      blade.position.applyAxisAngle(new THREE.Vector3(0, 1, 0), index * Math.PI * 0.4);
      this.fan.add(blade);
    }
    fanRotorMount.add(this.fan);
    this.carriageGroup.add(fanRotorMount);
    ...
    const controlTrim = box(1.3, 0.16, 0.08, orange);
    controlTrim.position.set(2.25, 0.54, 1.69);
    ...
    const filament = cylinder(0.42, 0.34, orange, 36);
    filament.rotation.z = Math.PI / 2;
    spool.add(filament);
    for (const x of [-0.2, 0.2]) {
      const flange = cylinder(0.55, 0.065, black, 36);
      flange.rotation.z = Math.PI / 2;
      flange.position.x = x;
      spool.add(flange);
    }
    const hub = cylinder(0.15, 0.48, steel, 28);
  ...
  setProbe(probe) {
    this.probeDeployed = Boolean(probe?.deployed);
  }

  depositFilament(axes) {
    const point = new THREE.Vector3(
      (axes.x - 117.5) * MM,
      0.855 + Math.max(0, axes.z) * MM,
      (axes.y - 117.5) * MM,
    );

    if (this.lastFilamentPoint && this.filamentSegmentCount < MAX_FILAMENT_SEGMENTS) {
      const distance = point.distanceTo(this.lastFilamentPoint);
      if (distance > 0.0005 && distance < 0.3) {
        const offset = this.filamentSegmentCount * 6;
        this.lastFilamentPoint.toArray(this.filamentPositions, offset);
        point.toArray(this.filamentPositions, offset + 3);
        this.filamentSegmentCount += 1;
        this.filamentGeometry.attributes.position.needsUpdate = true;
        this.filamentGeometry.setDrawRange(0, this.filamentSegmentCount * 2);
      }
    }
    this.lastFilamentPoint = point;
  }
  ...
  updateAxisTransforms() {
    ...
    this.bedGroup.position.z = BED_CENTER_Z - y;
    this.gantryGroup.position.set(0, 2.02 + z, -1.62);
  ...
  stepVisuals() {
    ...
    this.probePin.position.y = THREE.MathUtils.lerp(
      this.probePin.position.y,
      this.probeDeployed ? -1.03 : -0.88,
      0.28,
    );
  }
  ...
  animate(timestamp = 0) {
    ...
    if (timestamp - this.lastRenderTime >= 30) {
