...
const SECTOR_COUNT = 8192;
const RESERVED_SECTORS = 1;
const FAT_COUNT = 2;
const FAT_SECTORS = 32;
const ROOT_ENTRIES = 512;
const ROOT_SECTORS = (ROOT_ENTRIES * 32) / SECTOR_SIZE;
const FIRST_ROOT_SECTOR = RESERVED_SECTORS + FAT_COUNT * FAT_SECTORS;
const FIRST_DATA_SECTOR = FIRST_ROOT_SECTOR + ROOT_SECTORS;
...
function demoGcode() {
  ...
  for (let index = 0; index < 80; index += 1) {
    lines.push(index % 2 === 0 ? 'G1 X5 Y2 F3000' : 'G1 X-5 Y-2 F3000');
  }
  lines.push('G90', 'M84', '');
...
function buildFat16Image(name, content) {
  const image = new Uint8Array(SECTOR_COUNT * SECTOR_SIZE);
  ...
  write16(boot, 19, SECTOR_COUNT);
  boot[21] = 0xf8;
  write16(boot, 22, FAT_SECTORS);
  write16(boot, 24, 32);
  write16(boot, 26, 64);
  write32(boot, 28, 0);
  write32(boot, 32, 0);
  ...
  for (let copy = 0; copy < FAT_COUNT; copy += 1) {
    const fatOffset = (RESERVED_SECTORS + copy * FAT_SECTORS) * SECTOR_SIZE;
  ...
  const rootOffset = FIRST_ROOT_SECTOR * SECTOR_SIZE;
  ...
  write16(image, entry + 26, 2);
  write32(image, entry + 28, content.length);

  const dataOffset = FIRST_DATA_SECTOR * SECTOR_SIZE;
  image.set(content.subarray(0, image.length - dataOffset), dataOffset);
  return image;
}
...
export class VirtualSDCard {
  constructor() {
    ...
    this.image = buildFat16Image(this.fileName, demoGcode());
    this.selected = false;
    this.idle = true;
    this.command = [];
    this.response = [];
  ...
  mountFile(name, content) {
    ...
    this.image = buildFat16Image(name, content);
    this.selected = false;
    this.idle = true;
    this.appCommand = false;
    this.multiReadBlock = null;
  ...
  processCommand(bytes) {
    ...
    } else if (command === 41 && this.appCommand) {
      this.appCommand = false;
      this.idle = false;
