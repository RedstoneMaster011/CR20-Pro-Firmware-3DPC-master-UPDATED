...
#include "../feature/temperature_override.h"
#include "../feature/first_layer_control.h"
...
typedef struct SettingsDataStruct {
  ...
  //
  // Slicer temperature override
  //
  bool override_slicer_temp_settings;
  int16_t override_slicer_hotend_temp, override_slicer_bed_temp;

  //
  // Better Bed first-layer control
  //
  bool better_bed_enabled;
...
#include "../core/debug_out.h"
  ...
  bool MarlinSettings::save() {
    ...
    // Slicer temperature override
    //
    _FIELD_TEST(override_slicer_temp_settings);
    EEPROM_WRITE(override_slicer_temp_settings);
    EEPROM_WRITE(override_slicer_hotend_temp);
    EEPROM_WRITE(override_slicer_bed_temp);

    _FIELD_TEST(better_bed_enabled);
    EEPROM_WRITE(better_bed_enabled);
  ...
  bool MarlinSettings::_load() {
    ...
    const bool legacy_v67 = strncmp(stored_ver, "V67", 3) == 0,
               legacy_v68 = strncmp(stored_ver, "V68", 3) == 0,
               legacy_v69 = strncmp(stored_ver, "V69", 3) == 0,
               legacy_settings = legacy_v67 || legacy_v68 || legacy_v69;

    // Version has to match or defaults are used
    if (!legacy_settings && strncmp(version, stored_ver, 3) != 0) {
      if (stored_ver[3] != '\0') {
        stored_ver[0] = '?';
        stored_ver[1] = '\0';
      }
      ...
      {
        _FIELD_TEST(override_slicer_temp_settings);
        if (legacy_v67 || legacy_v68) {
          if (!validating) {
            override_slicer_temp_settings = SLICER_TEMP_OVERRIDE_DEFAULT;
            override_slicer_hotend_temp = SLICER_OVERRIDE_HOTEND_TEMP;
            override_slicer_bed_temp = SLICER_OVERRIDE_BED_TEMP;
          }
        }
        else {
          EEPROM_READ(override_slicer_temp_settings);
          EEPROM_READ(override_slicer_hotend_temp);
          EEPROM_READ(override_slicer_bed_temp);
        }
      }

      _FIELD_TEST(better_bed_enabled);
      if (legacy_settings) {
        if (!validating) better_bed_enabled = BETTER_BED_DEFAULT;
      }
      else
        EEPROM_READ(better_bed_enabled);

      const uint16_t read_size = eeprom_index - (EEPROM_OFFSET),
                     override_missing = legacy_v67 || legacy_v68 ? sizeof(bool) + sizeof(int16_t) * 2 : 0,
                     better_bed_missing = legacy_settings ? sizeof(bool) : 0,
                     expected_size = datasize() - override_missing - better_bed_missing - (legacy_v67 ? sizeof(bool) : 0);
    ...
    if (!validating && !eeprom_error && legacy_settings) {
...
void MarlinSettings::reset() {
  ...
  override_slicer_hotend_temp = SLICER_OVERRIDE_HOTEND_TEMP;
  override_slicer_bed_temp = SLICER_OVERRIDE_BED_TEMP;
  better_bed_enabled = BETTER_BED_DEFAULT;
