...
bool send_ok[BUFSIZE];
  ...
  static constexpr int16_t NO_SD_LAYER_MARKER = -1;
  static int16_t queued_sd_layer[BUFSIZE], pending_sd_layer = NO_SD_LAYER_MARKER;
#endif
...
GCodeQueue::GCodeQueue() {
  ...
  for (uint8_t i = 0; i < COUNT(send_ok); i++) {
    send_ok[i] = true;
    #if ENABLED(SDSUPPORT)
      queued_sd_layer[i] = NO_SD_LAYER_MARKER;
    #endif
  }
}
...
void GCodeQueue::clear() {
  ...
  #if ENABLED(SDSUPPORT)
    pending_sd_layer = NO_SD_LAYER_MARKER;
  #endif
}
...
void GCodeQueue::_commit_command(bool say_ok
  ...
  #if ENABLED(SDSUPPORT)
    queued_sd_layer[index_w] = pending_sd_layer;
    pending_sd_layer = NO_SD_LAYER_MARKER;
  #endif
  #if NUM_SERIAL > 1
    port[index_w] = p;
...
void GCodeQueue::get_serial_commands() {
  ...
  inline void GCodeQueue::get_sdcard_commands() {
    ...
    while (length < BUFSIZE && !card_eof && !stop_buffering) {
      ...
      card_eof = card.eof() || n == -1;
      const bool line_end = sd_char == '\n' || sd_char == '\r'
      ...
      if (n != -1 && !line_end) {
        if (sd_char == ';') {
          ...
          first_layer_control.begin_comment();
        }
        #if ENABLED(PAREN_COMMENTS)
          ...
          first_layer_control.comment_char(sd_char);
        else if (!sd_comment_mode
          #if ENABLED(PAREN_COMMENTS)
            && !sd_comment_paren_mode
          #endif
        ) {
          if (sd_count < MAX_CMD_SIZE - 1)
            buffer[index_w][sd_count++] = sd_char;
        }
      }

      if (!line_end && !card_eof) continue;

      if (sd_comment_mode) {
        const int16_t layer = first_layer_control.end_comment();
        if (layer >= 0) pending_sd_layer = layer;
      }
...
void GCodeQueue::advance() {
  ...
  #if ENABLED(SDSUPPORT)
    if (queued_sd_layer[index_r] >= 0) {
      first_layer_control.on_layer_marker(queued_sd_layer[index_r]);
      queued_sd_layer[index_r] = NO_SD_LAYER_MARKER;
    }
  #endif
