...
#include "../../feature/print_summary.h"
  ...
  static bool get_remaining_print_time(const duration_t &elapsed, const uint8_t progress, duration_t &remaining) {
    static uint32_t estimated_total_seconds, last_estimate_seconds, last_elapsed_seconds, last_position;
    static bool last_was_sd;

    const bool timer_active = print_job_timer.isRunning() || print_job_timer.isPaused();
    #if ENABLED(SDSUPPORT)
      const bool sd_print = card.isFileOpen() && timer_active;
    #else
      constexpr bool sd_print = false;
    #endif
    const bool active_print = sd_print || (print_summary_host_active() && timer_active);
    const uint32_t elapsed_seconds = elapsed.value,
                   position = sd_print ? card.getIndex() : progress,
                   total = sd_print ? card.getFileSize() : 100UL;

    if (!active_print || elapsed_seconds < last_elapsed_seconds || position < last_position || sd_print != last_was_sd) {
    ...
    last_was_sd = sd_print;

    if (!active_print || !progress || !position || !total)
      return false;

    if (!estimated_total_seconds || elapsed_seconds - last_estimate_seconds >= 60) {
      const uint32_t new_total = (uint32_t)(float(elapsed_seconds) * float(total) / float(position) + 0.5f);
