...
void menu_temperature() {
  ...
  MENU_ITEM_EDIT(bool, MSG_BETTER_BED, &better_bed_enabled);
  MENU_ITEM_EDIT(bool, MSG_OVERRIDE_SLICER_TEMPS, &override_slicer_temp_settings);
  if (override_slicer_temp_settings) {
    #if HAS_TEMP_HOTEND
      MENU_ITEM_EDIT(int3, MSG_OVERRIDE_SLICER_NOZZLE, &override_slicer_hotend_temp, HEATER_0_MINTEMP, HEATER_0_MAXTEMP - 15);
    #endif
    #if HAS_HEATED_BED
      MENU_ITEM_EDIT(int3, MSG_OVERRIDE_SLICER_BED, &override_slicer_bed_temp, BED_MINTEMP, BED_MAXTEMP - 10);
    #endif
  }
