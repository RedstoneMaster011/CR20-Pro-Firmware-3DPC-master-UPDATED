...
`Temperature > Better Bed` is enabled by default. For SD-card and USB prints, it sets the temporary `Tune > Speed` value to `70%` and `Tune > Fan Speed` to full (`255`) on layers 0 and 1. When layer 2 begins, Tune speed returns to `100%` and the firmware restores the most recent fan speed requested by the slicer. Cura layer comments are detected directly from SD files; extrusion-height changes provide the fallback for other files and USB streaming. Disable `Better Bed` before a print to leave both Tune values under normal slicer control.
...
- The progress bar fills from SD-card file progress or from `M73` percentages sent by a USB host.
...
When a print completes normally, the LCD opens `Print Finished` and shows the final elapsed time and an estimated filament weight in grams. SD prints use the slicer's `Filament used` header when present; otherwise the firmware estimates from accepted extrusion. Press the knob to return to the normal status screen. A stopped or canceled print does not show this success screen.

For USB printing, the host should send `M75` at the start, `M73 P<percent>` as progress changes, and `M77` after all print moves finish. Common slicer heater-wait and heater-off commands also provide automatic start/end detection, but explicit job commands give the clearest timing and progress behavior.
...
The estimate requires an active SD-card print or an active USB print whose host sends `M73` percentages. It remains on `Calc.` until the progress bar first visibly fills. Very short files may finish before a useful estimate can be calculated.
